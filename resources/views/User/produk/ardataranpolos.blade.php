...
<body>
    <div class="container">
        ...
        @php
            ...
            $place = $product->position_ar == 'dinding' ? 'wall' : 'floor';
        @endphp

        <model-viewer id="color" src="{{ $product->link_ar }}" ios-src="{{ $product->link_src }}"
            skybox-image="{{ $skybox }}" ar-placement="{{ $place }}" ar-modes="{{ $mode }}"
            shadow-intensity="{{ $product->bayangan ?? 1 }}" ar xr-environment ar-scale="auto"
            skybox-height="@if ($product->position_ar == 'dinding') 0 @else 1.8m @endif" camera-controls touch-action="pan-y"
            style=" width: 100%;
                            height: 450px;
                            border-radius: 10px;
                            position: relative;">
    ...
    <script>
        ...
        // Initialize model-viewer element and hide the default AR button
