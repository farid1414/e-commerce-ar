<nav class="navbar navbar-expand-lg navbar-light bg-light fixed-bottom" style="border-style: solid; border-color: rgba(245, 245, 220);">
  <div class="container-fluid">
      <ul class="navbar-nav flex-row w-100 d-flex justify-content-around">
          <li class="nav-item">
          ...
          <li class="nav-item">
              <a class="nav-link" href="/Listseluruhproduk">
                  <div class="d-flex flex-column align-items-center">
...
<script>
  ...
  function handleSearch() {
      ...
      if (searchTerm !== "" && searchResults.length > 0) {
          ...
          resultCount.innerHTML = `<span>Hasil Pencarian <b>"${searchTerm}"</b></span> <span class="text-muted">${searchResults.length} produk</span>`;
              ...
              <div class="card-body d-flex align-items-center">
                  ...
                  <div style="flex: 1;">
                      <div class="d-flex justify-content-between">
                          ...
                          ${produk.bintang ? `<span class="text-muted"><i class="bi bi-star-fill" style="color: gold;"></i> ${produk.bintang}</span>` : ""}
                      </div>
                      ${produk.varian ? `<span class="text-muted">${produk.varian}</span>` : ""}
                      <div class="d-flex justify-content-between">
                          ${produk.harga ? `<span class="text-muted">${produk.harga}</span>` : ""}
                          ${produk.terjual ? `<span class="text-muted">${produk.terjual}</span>` : ""}
                      </div>
                  </div>
              </div>
          `;
